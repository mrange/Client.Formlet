...
module Main =
    ...
    module RegistrationFlow = 
        ...
        let entityFlow =
            flowlet {
                let! fef, fpf           = showPage selectForm
                let! deliveryAddress    = showPage <| addressForm "Delivery address" "" ""
                let! entity             = fef deliveryAddress.FirstName deliveryAddress.LastName
                let! payment            = fpf deliveryAddress.FirstName deliveryAddress.LastName
                return
                    {
                        Entity              = entity
...
//        window.Content <- FormletControl.Create submit cancel complete
        window.Content <- FlowletControl.Create submit cancel RegistrationFlow.entityFlow RegistrationFlow.lastPage
